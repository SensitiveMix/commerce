...
(function (b) {
  var m, u, x, g, D, i, z, A, B, p = 0, e = {}, q = [], n = 0, c = {}, j = [], E = null, s = new Image(), G = /\.(jpg|gif|png|bmp|jpeg)(.*)?$/i, S = /[^\.]\.(swf)\s*$/i, H, I = 1, k, l, h = false, y = b.extend(b('<div/>')[0], {prop: 0}), v = 0, O = !b.support.opacity && !window.XMLHttpRequest, J = function () { u.hide(); s.onerror = s.onload = null; E && E.abort(); m.empty() }, P = function () { b.fancybox('<p id="fancybox_error">The requested content cannot be loaded.<br />Please try again later.</p>', {scrolling: 'no', padding: 20, transitionIn: 'none', transitionOut: 'none'}) },
    K = function () { return [b(window).width(), b(window).height(), b(document).scrollLeft(), b(document).scrollTop()] }, T = function () {
      ...
      }d.top = a[3] + (a[1] - (d.height + 40)) * 0.5; d.left = a[2] + (a[0] - (d.width + 40)) * 0.5; if (c.autoScale === false) { d.top = Math.max(a[3] + f, d.top); d.left = Math.max(a[2] + f, d.left) } return d
    }, U = function (a) {
      if (a && a.length) {
...
b.fancybox.center); c.centerOnScroll ? b(window).bind('scroll.fb', b.fancybox.center) : b(window).unbind('scroll.fb'); b.isFunction(c.onComplete) && c.onComplete(j, n, c); h = false; X()
      ...
      var d = Math.round(k.width + (l.width - k.width) * a), f = Math.round(k.height + (l.height - k.height) * a), o = Math.round(k.top + (l.top - k.top) * a), t = Math.round(k.left + (l.left - k.left) * a); g.css({width: d + 'px', height: f + 'px', top: o + 'px', left: t + 'px'}); d = Math.max(d - c.padding * 2, 0); f = Math.max(f - (c.padding * 2 + v * a), 0); i.css({width: d + 'px',
        height: f +
'px'}); if (typeof l.opacity !== 'undefined')g.css('opacity', a < 0.5 ? 0.5 : a)
      ...
      var a = e.orig ? b(e.orig) : false, d = {}; if (a && a.length) {
        a = Y(a); d = {width: a.width + c.padding * 2,
...
2, 1),
              height: Math.max(l.height - c.padding * 2 - v, 1)}); f()
        ...
        g.css('opacity', 1); if (c.transitionIn == 'elastic') {
          k = Q(); i.css({top: c.padding, left: c.padding, width: Math.max(k.width - c.padding * 2, 1), height: Math.max(k.height - c.padding * 2, 1)}).html(m.contents()); g.css(k).show(); if (c.opacity) {
            l.opacity =
0
          }y.prop = 0; b(y).animate({prop: 1}, {duration: c.speedIn, easing: c.easingIn, step: M, complete: L})
      ...
      h = true; e.width = s.width; e.height = s.height; b('<img />').attr({id: 'fancybox-img',
        src: s.src,
        alt: e.title}).appendTo(m); N()
    }, C = function () {
      J(); var a = q[p], d, f, o, t, w; e = b.extend({}, b.fn.fancybox.defaults, typeof b(a).data('fancybox') === 'undefined' ? e : b(a).data('fancybox')); o = a.title || b(a).title || e.title || ''; if (a.nodeName && !e.orig)e.orig = b(a).children('img:first').length ? b(a).children('img:first') : b(a); if (o === '' && e.orig)o = e.orig.attr('alt'); d = a.nodeName && /^(?:javascript|#)/i.test(a.href) ? e.href || null : e.href || a.href || null; if (e.type) { f = e.type; if (!d)d = e.content } else if (e.content)f = 'html'; else if (d) {
        if (d.match(G)) {
          f =
'image'
        } else if (d.match(S))f = 'swf'; else if (b(a).hasClass('iframe'))f = 'iframe'; else if (d.match(/#/)) { a = d.substr(d.indexOf('#')); f = b(a).length > 0 ? 'inline' : 'ajax' } else f = 'ajax'
      } else f = 'inline'; e.type = f; e.href = d; e.title = o; if (e.autoDimensions && e.type !== 'iframe' && e.type !== 'swf') { e.width = 'auto'; e.height = 'auto' } if (e.modal) { e.overlayShow = true; e.hideOnOverlayClick = false; e.hideOnContentClick = false; e.enableEscapeButton = false; e.showCloseButton = false } if (b.isFunction(e.onStart)) {
...
1)
    ...
    if (!(h || g.is(':hidden'))) {
      h = true; if (c && b.isFunction(c.onCleanup)) if (c.onCleanup(j, n, c) === false) { h = false; return }J(); b(z.add(A).add(B)).hide(); b('#fancybox-title').remove(); g.add(i).add(x).unbind(); b(window).unbind('resize.fb scroll.fb'); b(document).unbind('keydown.fb'); i.css('overflow', 'hidden'); if (c.transitionOut == 'elastic') {
        k = Q(); var d = g.position(); l = {top: d.top, left: d.left, width: g.width(), height: g.height()}; if (c.opacity)l.opacity = 1; y.prop = 1; b(y).animate({prop: 0}, {duration: c.speedOut,
          easing: c.easingOut,
          step: M,
          complete: a})
...
d, f.left); g.css(f); h = false
  }; b.fn.fancybox.defaults = {padding: 10,
    margin: 20,
    opacity: false,
    modal: false,
    cyclic: false,
    ...
    onComplete: null,
    onCleanup: null,
    onClosed: null}; b(document).ready(function () { aa() })
})(jQuery)
