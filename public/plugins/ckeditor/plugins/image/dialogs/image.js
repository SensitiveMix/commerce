...
(function () {
  var r = function (c, j) {
    function r () { var a = arguments, b = this.getContentElement('advanced', 'txtdlgGenStyle'); b && b.commit.apply(b, a); this.foreach(function (b) { b.commit && b.id != 'txtdlgGenStyle' && b.commit.apply(b, a) }) } function i (a) { if (!s) { s = 1; var b = this.getDialog(), d = b.imageElement; if (d) { this.commit(f, d); for (var a = [].concat(a), e = a.length, c, g = 0; g < e; g++)(c = b.getContentElement.apply(b, a[g].split(':'))) && c.setup(f, d) }s = 0 } } var f = 1, k = /^\s*(\d+)((px)|\%)?\s*$/i, v = /(^\s*(\d+)((px)|\%)?\s*$)|^$/i, o = /^\d+px$/,
      w = function () { var a = this.getValue(), b = this.getDialog(), d = a.match(k); d && (d[2] == '%' && l(b, !1), a = d[1]); b.lockRatio && (d = b.originalElement, d.getCustomData('isReady') == 'true' && (this.id == 'txtHeight' ? (a && a != '0' && (a = Math.round(d.$.width * (a / d.$.height))), isNaN(a) || b.setValueOf('info', 'txtWidth', a)) : (a && a != '0' && (a = Math.round(d.$.height * (a / d.$.width))), isNaN(a) || b.setValueOf('info', 'txtHeight', a)))); g(b) }, g = function (a) { if (!a.originalElement || !a.preview) return 1; a.commitContent(4, a.preview); return 0 }, s, l = function (a,
b) {
        if (!a.getContentElement('info', 'ratioLock')) return null; var d = a.originalElement; if (!d) return null; if (b == 'check') { if (!a.userlockRatio && d.getCustomData('isReady') == 'true') { var e = a.getValueOf('info', 'txtWidth'), c = a.getValueOf('info', 'txtHeight'), d = 1E3 * d.$.width / d.$.height, f = 1E3 * e / c; a.lockRatio = !1; !e && !c ? a.lockRatio = !0 : !isNaN(d) && !isNaN(f) && Math.round(d) == Math.round(f) && (a.lockRatio = !0) } } else void 0 != b ? a.lockRatio = b : (a.userlockRatio = 1, a.lockRatio = !a.lockRatio); e = CKEDITOR.document.getById(p); a.lockRatio
? e.removeClass('cke_btn_unlocked') : e.addClass('cke_btn_unlocked'); e.setAttribute('aria-checked', a.lockRatio); CKEDITOR.env.hc && e.getChild(0).setHtml(a.lockRatio ? CKEDITOR.env.ie ? '■' : '▣' : CKEDITOR.env.ie ? '□' : '▢'); return a.lockRatio
      }, x = function (a) { var b = a.originalElement; if (b.getCustomData('isReady') == 'true') { var d = a.getContentElement('info', 'txtWidth'), e = a.getContentElement('info', 'txtHeight'); d && d.setValue(b.$.width); e && e.setValue(b.$.height) }g(a) }, y = function (a, b) {
        function d (a, b) {
          var d = a.match(k); return d
? (d[2] == '%' && (d[1] += '%', l(e, !1)), d[1]) : b
        } if (a == f) { var e = this.getDialog(), c = '', g = this.id == 'txtWidth' ? 'width' : 'height', h = b.getAttribute(g); h && (c = d(h, c)); c = d(b.getStyle(g), c); this.setValue(c) }
      }, t, q = function () {
...
0, this); this.dontResetSize = this.firstLoad = !1
      ...
      z = n('previewImage'); return {title: c.lang.image[j == 'image' ? 'title' : 'titleButton'],
        minWidth: 420,
        minHeight: 360,
        onShow: function () {
          ...
          this.preview = CKEDITOR.document.getById(z); this.originalElement = a.document.createElement('img'); this.originalElement.setAttribute('alt', ''); this.originalElement.setCustomData('isReady', 'false'); if (d) {
            this.linkElement = d; this.linkEditMode = !0; var c = d.getChildren(); if (c.count() == 1) { var g = c.getItem(0).getName(); if (g == 'img' || g == 'input') this.imageElement = c.getItem(0), this.imageElement.getName() == 'img' ? this.imageEditMode = 'img' : this.imageElement.getName() == 'input' && (this.imageEditMode = 'input') }j == 'image' &&
this.setupContent(2, d)
          } if (this.customImageElement) this.imageEditMode = 'img', this.imageElement = this.customImageElement, delete this.customImageElement; else if (b && b.getName() == 'img' && !b.data('cke-realelement') || b && b.getName() == 'input' && b.getAttribute('type') == 'image') this.imageEditMode = b.getName(), this.imageElement = b; this.imageEditMode ? (this.cleanImageElement = this.imageElement, this.imageElement = this.cleanImageElement.clone(!0, !0), this.setupContent(f, this.imageElement)) : this.imageElement = a.document.createElement('img')
        ...
        onOk: function () {
          if (this.imageEditMode) {
            var a = this.imageEditMode; j == 'image' && a == 'input' && confirm(c.lang.image.button2Img) ? (this.imageElement = c.document.createElement('img'), this.imageElement.setAttribute('alt', ''), c.insertElement(this.imageElement)) : j != 'image' && a == 'img' && confirm(c.lang.image.img2Button) ? (this.imageElement = c.document.createElement('input'),
this.imageElement.setAttributes({type: 'image', alt: ''}), c.insertElement(this.imageElement)) : (this.imageElement = this.cleanImageElement, delete this.cleanImageElement)
        ...
        onLoad: function () { j != 'image' && this.hidePage('Link'); var a = this._.element.getDocument(); this.getContentElement('info', 'ratioLock') && (this.addFocusable(a.getById(u), 5), this.addFocusable(a.getById(p), 5)); this.commitContent = r },
        onHide: function () {
      ...
      children: [{type: 'vbox',
        ...
        children: [{type: 'text',
          ...
          validate: function () {
            ...
            (a = !!(a && parseInt(a[1], 10) !== 0)) || alert(c.lang.common.invalidWidth); return a
          },
          setup: y,
          commit: function (a, b, d) { var c = this.getValue(); a == f ? (c ? b.setStyle('width', CKEDITOR.tools.cssLength(c)) : b.removeStyle('width'), !d && b.removeAttribute('width')) : a == 4 ? c.match(k) ? b.setStyle('width', CKEDITOR.tools.cssLength(c)) : (a = this.getDialog().originalElement, a.getCustomData('isReady') == 'true' && b.setStyle('width', a.$.width + 'px')) : a == 8 && (b.removeAttribute('width'), b.removeStyle('width')) }}, {type: 'text',
            id: 'txtHeight',
            width: '45px',
            label: c.lang.common.height,
            onKeyUp: w,
            onChange: function () { i.call(this, 'advanced:txtdlgGenStyle') },
            validate: function () { var a = this.getValue().match(v); (a = !!(a && parseInt(a[1], 10) !== 0)) || alert(c.lang.common.invalidHeight); return a },
            setup: y,
            commit: function (a, b, d) {
              var c = this.getValue(); a == f ? (c ? b.setStyle('height', CKEDITOR.tools.cssLength(c)) : b.removeStyle('height'), !d && b.removeAttribute('height')) : a == 4 ? c.match(k) ? b.setStyle('height', CKEDITOR.tools.cssLength(c)) : (a = this.getDialog().originalElement,
a.getCustomData('isReady') == 'true' && b.setStyle('height', a.$.height + 'px')) : a == 8 && (b.removeAttribute('height'), b.removeStyle('height'))
            }}]}, {id: 'ratioLock',
              type: 'html',
...
'" class="cke_btn_locked" id="' + p + '" role="checkbox"><span class="cke_icon"></span><span class="cke_label">' + c.lang.image.lockRatio + '</span></a><a href="javascript:void(0)" tabindex="-1" title="' + c.lang.image.resetSize + '" class="cke_btn_reset" id="' + u + '" role="button"><span class="cke_label">' + c.lang.image.resetSize + '</span></a></div>'}]}, {type: 'vbox',
  ...
  children: [{type: 'text',
    ...
    setup: function (a, b) { if (a == f) { var d; d = (d = (d = b.getStyle('border-width')) && d.match(/^(\d+px)(?: \1 \1 \1)?$/)) && parseInt(d[1], 10); isNaN(parseInt(d, 10)) && (d = b.getAttribute('border')); this.setValue(d) } },
    commit: function (a, b, d) {
      var c = parseInt(this.getValue(), 10); a == f || a == 4 ? (isNaN(c) ? !c && this.isChanged() && b.removeStyle('border') : (b.setStyle('border-width', CKEDITOR.tools.cssLength(c)),
b.setStyle('border-style', 'solid')), !d && a == f && b.removeAttribute('border')) : a == 8 && (b.removeAttribute('border'), b.removeStyle('border-width'), b.removeStyle('border-style'), b.removeStyle('border-color'))
    }}, {type: 'text',
      id: 'txtHSpace',
      ...
      setup: function (a, b) { if (a == f) { var d, c; d = b.getStyle('margin-left'); c = b.getStyle('margin-right'); d = d && d.match(o); c = c && c.match(o); d = parseInt(d, 10); c = parseInt(c, 10); d = d == c && d; isNaN(parseInt(d, 10)) && (d = b.getAttribute('hspace')); this.setValue(d) } },
      commit: function (a, b, c) {
        var e = parseInt(this.getValue(), 10); a == f || a == 4 ? (isNaN(e) ? !e && this.isChanged() && (b.removeStyle('margin-left'), b.removeStyle('margin-right')) : (b.setStyle('margin-left', CKEDITOR.tools.cssLength(e)), b.setStyle('margin-right', CKEDITOR.tools.cssLength(e))),
!c && a == f && b.removeAttribute('hspace')) : a == 8 && (b.removeAttribute('hspace'), b.removeStyle('margin-left'), b.removeStyle('margin-right'))
      }}, {type: 'text',
        id: 'txtVSpace',
...
b.getStyle('margin-bottom'); c = c && c.match(o); e = e && e.match(o); c = parseInt(c, 10); e = parseInt(e, 10); c = c == e && c; isNaN(parseInt(c, 10)) && (c = b.getAttribute('vspace')); this.setValue(c)
        ...
        commit: function (a, b, c) {
          var e = parseInt(this.getValue(), 10); a == f || a == 4 ? (isNaN(e) ? !e && this.isChanged() && (b.removeStyle('margin-top'), b.removeStyle('margin-bottom')) : (b.setStyle('margin-top', CKEDITOR.tools.cssLength(e)), b.setStyle('margin-bottom', CKEDITOR.tools.cssLength(e))), !c && a == f && b.removeAttribute('vspace')) : a == 8 && (b.removeAttribute('vspace'),
  ...
  elements: [{id: 'txtUrl',
    ...
    commit: function (a, b) {
      if (a == 2 && (this.getValue() || this.isChanged())) {
        var d = decodeURI(this.getValue()); b.data('cke-saved-href', d); b.setAttribute('href', d); if (this.getValue() || !c.config.image_removeLinkByEmptyURL) {
          this.getDialog().addLink =
!0
        }
      }
    }}, {type: 'button', id: 'browse', filebrowser: {action: 'Browse', target: 'Link:txtUrl', url: c.config.filebrowserImageBrowseLinkUrl}, style: 'float:right', hidden: !0, label: c.lang.common.browseServer}, {id: 'cmbTarget',
      type: 'select',
      ...
      setup: function (a, b) {
        a == 2 && this.setValue(b.getAttribute('target') ||
'')
      },
      commit: function (a, b) { a == 2 && (this.getValue() || this.isChanged()) && b.setAttribute('target', this.getValue()) }}]}, {id: 'Upload', hidden: !0, filebrowser: 'uploadButton', label: c.lang.image.upload, elements: [{type: 'file', id: 'upload', label: c.lang.image.btnUpload, style: 'height:40px', size: 38}, {type: 'fileButton', id: 'uploadButton', filebrowser: 'info:txtUrl', label: c.lang.image.btnUpload, 'for': ['Upload', 'upload']}]}, {id: 'advanced',
        label: c.lang.common.advancedTab,
        elements: [{type: 'hbox',
          widths: ['50%', '25%', '25%'],
          children: [{type: 'text', id: 'linkId', requiredContent: 'img[id]', label: c.lang.common.id, setup: function (a, b) { a == f && this.setValue(b.getAttribute('id')) }, commit: function (a, b) { a == f && (this.getValue() || this.isChanged()) && b.setAttribute('id', this.getValue()) }}, {id: 'cmbLangDir',
            type: 'select',
