...
*/
(function () {
  function r (a) { for (var e = 0, l = 0, k = 0, m, g = a.$.rows.length; k < g; k++) { m = a.$.rows[k]; for (var d = e = 0, c, b = m.cells.length; d < b; d++)c = m.cells[d], e += c.colSpan; e > l && (l = e) } return l } function o (a) { return function () { var e = this.getValue(), e = !!(CKEDITOR.dialog.validate.integer()(e) && e > 0); e || (alert(a), this.select()); return e } } function n (a, e) {
    var l = function (g) { return new CKEDITOR.dom.element(g, a.document) }, n = a.editable(), m = a.plugins.dialogadvtab; return {title: a.lang.table.title,
      minWidth: 310,
      minHeight: CKEDITOR.env.ie
? 310 : 280,
      onLoad: function () { var g = this, a = g.getContentElement('advanced', 'advStyles'); if (a)a.on('change', function () { var a = this.getStyle('width', ''), b = g.getContentElement('info', 'txtWidth'); b && b.setValue(a, !0); a = this.getStyle('height', ''); (b = g.getContentElement('info', 'txtHeight')) && b.setValue(a, !0) }) },
      onShow: function () {
...
'txtHeight'); e == 'tableProperties' && ((g = g.getSelectedElement()) && g.is('table') ? c = g : d.length > 0 && (CKEDITOR.env.webkit && d[0].shrink(CKEDITOR.NODE_ELEMENT), c = a.elementPath(d[0].getCommonAncestor(!0)).contains('table', 1)), this._.selectedElement = c); c ? (this.setupContent(c), b && b.disable(), h && h.disable()) : (b && b.enable(), h && h.enable()); p && p.onChange(); f && f.onChange()
      },
      onOk: function () {
        var g = a.getSelection(), d = this._.selectedElement && g.createBookmarks(), c = this._.selectedElement || l('table'), b = {}; this.commitContent(b,
c); if (b.info) {
  b = b.info; if (!this._.selectedElement) for (var h = c.append(l('tbody')), e = parseInt(b.txtRows, 10) || 0, f = parseInt(b.txtCols, 10) || 0, i = 0; i < e; i++) for (var j = h.append(l('tr')), k = 0; k < f; k++)j.append(l('td')).appendBogus(); e = b.selHeaders; if (!c.$.tHead && (e == 'row' || e == 'both')) {
    j = new CKEDITOR.dom.element(c.$.createTHead()); h = c.getElementsByTag('tbody').getItem(0); h = h.getElementsByTag('tr').getItem(0); for (i = 0; i < h.getChildCount(); i++) {
      f = h.getChild(i), f.type == CKEDITOR.NODE_ELEMENT && !f.data('cke-bookmark') &&
(f.renameNode('th'), f.setAttribute('scope', 'col'))
    }j.append(h.remove())
  } if (c.$.tHead !== null && !(e == 'row' || e == 'both')) { j = new CKEDITOR.dom.element(c.$.tHead); h = c.getElementsByTag('tbody').getItem(0); for (k = h.getFirst(); j.getChildCount() > 0;) { h = j.getFirst(); for (i = 0; i < h.getChildCount(); i++)f = h.getChild(i), f.type == CKEDITOR.NODE_ELEMENT && (f.renameNode('td'), f.removeAttribute('scope')); h.insertBefore(k) }j.remove() } if (!this.hasColumnHeaders && (e == 'col' || e == 'both')) {
    for (j = 0; j < c.$.rows.length; j++) {
      f = new CKEDITOR.dom.element(c.$.rows[j].cells[0]),
f.renameNode('th'), f.setAttribute('scope', 'row')
    }
  } if (this.hasColumnHeaders && !(e == 'col' || e == 'both')) for (i = 0; i < c.$.rows.length; i++)j = new CKEDITOR.dom.element(c.$.rows[i]), j.getParent().getName() == 'tbody' && (f = new CKEDITOR.dom.element(j.$.cells[0]), f.renameNode('td'), f.removeAttribute('scope')); b.txtHeight ? c.setStyle('height', b.txtHeight) : c.removeStyle('height'); b.txtWidth ? c.setStyle('width', b.txtWidth) : c.removeStyle('width'); c.getAttribute('style') || c.removeAttribute('style')
} if (this._.selectedElement) try { g.selectBookmarks(d) } catch (m) {} else {
  a.insertElement(c),
        ...
        elements: [{type: 'hbox',
          ...
          children: [{type: 'vbox',
            ...
            children: [{type: 'text',
              ...
              commit: k}, {type: 'text', id: 'txtCols', 'default': 2, label: a.lang.table.columns, required: !0, controlStyle: 'width:5em', validate: o(a.lang.table.invalidCols), setup: function (a) { this.setValue(r(a)) }, commit: k}, {type: 'html', html: '&nbsp;'}, {type: 'select',
                id: 'selHeaders',
                requiredContent: 'th',
                'default': '',
          ...
          {type: 'vbox',
            ...
            children: [{type: 'hbox',
              ...
              children: [{type: 'text',
                ...
                'default': a.filter.check('table{width}') ? n.getSize('width') < 500 ? '100%' : 500 : 0,
                getValue: q,
