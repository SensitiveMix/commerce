...
{}; c.match(x) && (o == 'encode' ? c = c.replace(E, function (a, c, b) { return 'mailto:' + String.fromCharCode.apply(String, c.split(',')) + (b && b.replace(/\\'/g, "'")) }) : o && c.replace(F, function (a, c, d) { if (c == q) { b.type = 'email'; for (var a = b.email = {}, c = /(^')|('$)/g, d = d.match(/[^,\s]+/g), e = d.length, g, f, h = 0; h < e; h++)g = decodeURIComponent, f = d[h].replace(c, '').replace(/\\'/g, "'"), f = g(f), g = p[h].toLowerCase(), a[g] = f; a.address = [a.name, a.domain].join('@') } })); if (!b.type) {
  if (d = c.match(B)) {
    b.type = 'anchor', b.anchor = {}, b.anchor.name = b.anchor.id =
d[1]
  } else if (d = c.match(y)) { e = c.match(z); c = c.match(A); b.type = 'email'; var f = b.email = {}; f.address = d[1]; e && (f.subject = decodeURIComponent(e[1])); c && (f.body = decodeURIComponent(c[1])) } else c && (e = c.match(C)) ? (b.type = 'url', b.url = {}, b.url.protocol = e[1], b.url.url = e[2]) : b.type = 'url'
} if (g) {
  d = g.getAttribute('target'); b.target = {}; b.adv = {}; if (d)d.match(D) ? b.target.type = b.target.name = d : (b.target.type = 'frame', b.target.name = d); else if (d = (d = g.data('cke-pa-onclick') || g.getAttribute('onclick')) && d.match(G)) {
    b.target.type =
'popup'; for (b.target.name = d[1]; c = H.exec(d[2]);)(c[2] == 'yes' || c[2] == '1') && !(c[1] in {height: 1, width: 1, top: 1, left: 1}) ? b.target[c[1]] = !0 : isFinite(c[2]) && (b.target[c[1]] = c[2])
  }d = function (a, c) { var d = g.getAttribute(c); d !== null && (b.adv[a] = d || '') }; d('advId', 'id'); d('advLangDir', 'dir'); d('advAccessKey', 'accessKey'); b.adv.advName = g.data('cke-saved-name') || g.getAttribute('name') || ''; d('advLangCode', 'lang'); d('advTabIndex', 'tabindex'); d('advTitle', 'title'); d('advContentType', 'type'); CKEDITOR.plugins.link.synAnchorSelector
? b.adv.advCSSClasses = v(g) : d('advCSSClasses', 'class'); d('advCharset', 'charset'); d('advStyles', 'style'); d('advRel', 'rel')
  ...
  f = new CKEDITOR.dom.nodeList(a.document.$.anchors); c = 0; for (e = f.count(); c < e; c++) {
    h = f.getItem(c), d[c] =
...
''
    }, m = function (a) { a.adv || (a.adv = {}); a.adv[this.id] = this.getValue() || '' }, o = n.config.emailProtection || ''; o && o != 'encode' && (q = p = void 0, o.replace(/^([^(]+)\(([^)]+)\)$/, function (a, b, c) { q = b; p = []; c.replace(/[^,\s]+/g, function (a) { p.push(a) }) })); var i = n.lang.common, b = n.lang.link; return {title: b.title,
      minWidth: 350,
      minHeight: 230,
      contents: [{id: 'info',
        label: b.info,
...
{type: 'hbox', children: [{type: 'text', widths: ['50%', '50%'], labelLayout: 'horizontal', label: i.width, id: 'width', setup: j, commit: l}, {type: 'text', labelLayout: 'horizontal', widths: ['50%', '50%'], label: b.popupLeft, id: 'left', setup: j, commit: l}]}, {type: 'hbox', children: [{type: 'text', labelLayout: 'horizontal', widths: ['50%', '50%'], label: i.height, id: 'height', setup: j, commit: l}, {type: 'text', labelLayout: 'horizontal', label: b.popupTop, widths: ['50%', '50%'], id: 'top', setup: j, commit: l}]}]}]}]}, {id: 'upload',
  ...
  elements: [{type: 'file', id: 'upload', label: i.upload, style: 'height:40px', size: 29}, {type: 'fileButton', id: 'uploadButton', label: i.uploadSubmit, filebrowser: 'info:url', 'for': ['upload', 'upload']}]}, {id: 'advanced',
    ...
    elements: [{type: 'vbox',
      ...
      children: [{type: 'hbox',
        widths: ['45%', '35%', '20%'],
        children: [{type: 'text', id: 'advId', requiredContent: 'a[id]', label: b.id, setup: k, commit: m}, {type: 'select',
          id: 'advLangDir',
          ...
          setup: k,
          commit: m}, {type: 'text', id: 'advAccessKey', requiredContent: 'a[accesskey]', width: '80px', label: b.acccessKey, maxLength: 1, setup: k, commit: m}]}, {type: 'hbox',
            widths: ['45%', '35%', '20%'],
            children: [{type: 'text', label: b.name, id: 'advName', requiredContent: 'a[name]', setup: k, commit: m}, {type: 'text', label: b.langCode, id: 'advLangCode', requiredContent: 'a[lang]', width: '110px', 'default': '', setup: k, commit: m},
{type: 'text', label: b.tabIndex, id: 'advTabIndex', requiredContent: 'a[tabindex]', width: '80px', maxLength: 5, setup: k, commit: m}]}]}, {type: 'vbox',
  padding: 1,
  children: [{type: 'hbox', widths: ['45%', '55%'], children: [{type: 'text', label: b.advisoryTitle, requiredContent: 'a[title]', 'default': '', id: 'advTitle', setup: k, commit: m}, {type: 'text', label: b.advisoryContentType, requiredContent: 'a[type]', 'default': '', id: 'advContentType', setup: k, commit: m}]}, {type: 'hbox',
    widths: ['45%', '55%'],
    children: [{type: 'text',
      label: b.cssClasses,
      ...
      id: 'advCSSClasses',
      setup: k,
      commit: m}, {type: 'text', label: b.charset, requiredContent: 'a[charset]', 'default': '', id: 'advCharset', setup: k, commit: m}]}, {type: 'hbox', widths: ['45%', '55%'], children: [{type: 'text', label: b.rel, requiredContent: 'a[rel]', 'default': '', id: 'advRel', setup: k, commit: m}, {type: 'text', label: b.styles, requiredContent: 'a{cke-xyz}', 'default': '', id: 'advStyles', validate: CKEDITOR.dialog.validate.inlineStyle(n.lang.common.invalidInlineStyle), setup: k, commit: m}]}]}]}],
      onShow: function () { var a = this.getParentEditor(), b = a.getSelection(), c = null; (c = w.getSelectedLink(a)) && c.hasAttribute('href') ? b.getSelectedElement() || b.selectElement(c) : c = null; this.setupContent(I.apply(this, [a, c])) },
      onOk: function () {
