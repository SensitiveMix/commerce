    ...
    _.text = function () {
      ...
      return cmd.foldChildren(cmd.textTemplate[i], function (text, child) {
        ...
        if (text && cmd.textTemplate[i] === '(' &&
          child_text[0] === '(' && child_text.slice(-1) === ')') { return text + child_text.slice(1, -1) + cmd.textTemplate[i] }
 ...
 */
  var MathBlock = P(MathElement, function (_) {
    ...
    _.write = function (cursor, ch, replacedFragment) {
      ...
      { cmd = Variable(ch) } else if (cmd = CharCmds[ch] || LatexCmds[ch]) { cmd = cmd(ch) } else { cmd = VanillaSymbol(ch) }
  ...
  var RootMathCommand = P(MathCommand, function (_, _super) {
    ...
    this.ends[R] =
      ...
      this.ends[L].write = function (cursor, ch, replacedFragment) {
        if (ch !== '$') { MathBlock.prototype.write.call(this, cursor, ch, replacedFragment) } else if (this.isEmpty()) {
          cursor.insRightOf(this.parent).backspace().show()
          VanillaSymbol('\\$', '$').createLeftOf(cursor)
        } else if (!cursor[R]) { cursor.insRightOf(this.parent) } else if (!cursor[L]) { cursor.insLeftOf(this.parent) } else { MathBlock.prototype.write.call(this, cursor, ch, replacedFragment) }
      }
    }
    _.latex = function () {
...
// text-decoration
  ...
  var SupSub = P(MathCommand, function (_, _super) {
    ...
    _.latex = function () {
      ...
      if (latex.length === 1) { return this.ctrlSeq + latex } else { return this.ctrlSeq + '{' + (latex || ' ') + '}' }
    }
    _.redraw = function () {
...
// Closing bracket matching opening bracket above
  var CloseBracket = P(Bracket, function (_, _super) {
    ...
    // just put cursor after my parent
      if (!cursor[R] && cursor.parent.parent && cursor.parent.parent.end === this.end && !this.replacedFragment) { cursor.insRightOf(cursor.parent.parent) } else { _super.createLeftOf.call(this, cursor) }
    }
    _.placeCursor = function (cursor) {
      this.ends[L].blur()
...
LatexCmds.textmd = P(MathCommand, function (_, _super) {
  ...
  _.createLeftOf = function (cursor) {
    ...
    if (this.replacedText) {
      for (var i = 0; i < this.replacedText.length; i += 1) { this.ends[L].write(cursor, this.replacedText.charAt(i)) }
    }
  }
})
  ...
  // backspace and delete at ends of block don't unwrap
    ...
    _.write = function (cursor, ch, replacedFragment) {
      ...
      if (ch !== '$') {
        ...
        VanillaSymbol('\\$', '$').createLeftOf(cursor)
      } else if (!cursor[R]) { cursor.insRightOf(this.parent) } else if (!cursor[L]) { cursor.insLeftOf(this.parent) } else { // split apart
    ...
    _.blur = function () {
      ...
      if (this.isEmpty()) {
        ...
        if (cursor.parent === this) { this.jQ.addClass('empty') } else {
          cursor.hide()
          textblock.remove()
          if (cursor[R] === textblock) { cursor[R] = textblock[R] } else if (cursor[L] === textblock) { cursor[L] = textblock[L] }
    ...
    _.focus = function () {
      ...
      if (textblock[R].ctrlSeq === textblock.ctrlSeq) { // TODO: seems like there should be a better way to move MathElements around
        ...
        if (this.ends[R]) { this.ends[R][R] = rightward.ends[L] } else { this.ends[L] = rightward.ends[L] }
        ...
        if (cursor[L]) { cursor.insRightOf(cursor[L]) } else { cursor.insAtLeftEnd(this) }

        cursor.parent.bubble('redraw')
      } else if (textblock[L].ctrlSeq === textblock.ctrlSeq) {
        var cursor = textblock.cursor
        if (cursor[L]) { textblock[L].ends[L].focus() } else { cursor.insAtRightEnd(textblock[L].ends[L]) }
      }
      return this
    }
  })
  ...
  _.onKey = function (key, e) {
    ...
    if (currentBlock.parent === this) {
      if (key === 'Enter') { // enter
        ...
        if (currentBlock[R]) { currentBlock[R][L] = newBlock } else { this.ends[R] = newBlock }
        ...
        if (currentBlock.isEmpty()) {
          ...
          if (currentBlock[R]) { currentBlock[R][L] = currentBlock[L] } else { this.ends[R] = currentBlock[L] }
  ...
  var Variable = P(Symbol, function (_, _super) {
    ...
    _.text = function () {
      ...
      if (this[L] && !(this[L] instanceof Variable) &&
        !(this[L] instanceof BinaryOperator)) { text = '*' + text }
   ...
   */
    ...
    _.seek = function (target, pageX, pageY) {
      ...
      if (cmd instanceof Symbol) { // insert at whichever side is closer
        if (target.outerWidth() > 2 * (pageX - target.offset().left)) { cursor.insLeftOf(cmd) } else { cursor.insRightOf(cmd) }
      ...
      if (cmd) { cursor.insRightOf(cmd) } else { cursor.insAtRightEnd(block) }
    ...
    _.deleteDir = function (dir) {
      ...
      if (this.deleteSelection()); // pass
        ...
        if (this[dir].isEmpty()) { this[dir] = this[dir].remove()[dir] } else { this.selectDir(dir) }
      } else if (this.parent !== this.root) {
        if (this.parent.parent.isEmpty()) { return this.insDirOf(-dir, this.parent.parent).deleteDir(dir) } else { this.unwrapGramp() }
      }
